...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use NEXT_PUBLIC_API_URL or fallback to localhost for local development
      const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
      const res = await fetch(`${API_URL}/api/auth/signup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password, name, role: activeRole }),
      });
    ...
    <main className="flex-grow flex items-center justify-center p-6 min-h-screen bg-gradient-to-br from-[#e2e8f8] via-[#f9f9ff] to-[#f9f9ff]">
      ...
      <div className="w-full max-w-[440px] my-8">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Join us in connecting surplus food to those in need.</p>
        </div>

        {/* Auth Card */}
        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-xl shadow-gray-200/60">
          <div className="mb-6">
            <h2 className="text-xl font-bold text-[#151c27] mb-1">Create an account</h2>
            <p className="text-sm text-gray-500">Please fill in the details to get started.</p>
          </div>
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl border border-gray-200/50 gap-1">
              {(Object.keys(roleConfig) as Role[]).map((role) => (
                <button
                  ...
                  className={`flex-1 py-2.5 px-2 rounded-lg text-xs font-bold transition-all duration-200 ${activeRole === role
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            {/* Name */}
            <div>
              <label className="block text-sm font-semibold text-[#151c27] mb-2" htmlFor="name">
              ...
              <input
                ...
                placeholder="John Doe or Org Name"
            ...
            {/* Password */}
            <div>
              <label className="block text-sm font-semibold text-[#151c27] mb-2" htmlFor="password">
                Password
              </label>
            ...
            <div className="pt-4 space-y-3">
              ...
              <button
                type="button"
                className="w-full flex items-center justify-center gap-3 border border-gray-200 bg-white text-[#151c27] font-semibold text-sm py-3.5 rounded-xl hover:bg-gray-50 transition-colors duration-150"
              >
          ...
          <div className="mt-6 pt-5 border-t border-gray-100 text-center">
            <p className="text-sm text-gray-500">
              ...
              <Link href="/login" className="text-[#006c49] font-bold hover:underline ml-1">
                Sign in
              </Link>
            </p>
          </div>
